...
![image-20260721093454279](小土堆.assets/image-20260721093454279.png)

![image-20260721093509372](小土堆.assets/image-20260721093509372.png)

![image-20260721093528809](小土堆.assets/image-20260721093528809.png)



**为什么要取log**

![image-20260721093602081](小土堆.assets/image-20260721093602081.png)

![image-20260721093617841](小土堆.assets/image-20260721093617841.png)

![image-20260721093634253](小土堆.assets/image-20260721093634253.png)

这里“等价”是因为`log `是单调递增函数：原来的数越大，取 `log `后仍然越大，所以最大值对应的参数不会变化。



**为什么最大化会变成损失函数的最小化**

训练神经网络时，习惯使用：损失越小越好；但是最大似然要求：似然越大越好。

<img src="小土堆.assets/image-20260721094100013.png" alt="image-20260721094100013" style="zoom:50%;" />
...
![image-20260721094334723](小土堆.assets/image-20260721094334723.png)

![image-20260721094345074](小土堆.assets/image-20260721094345074.png)
...
![image-20260721095713188](小土堆.assets/image-20260721095713188.png)
...
**一张图片叫一个样本；一组图片叫一个 batch；完整看完一次训练集叫一个 epoch。**

1. 前向传播通常一次处理一个batch

   每执行一次 `outputs = model(img)` 会一次性计算这一批中的所有图片。

2. 一次完整的参数更新通常对应一个batch
   ...
   它处理的是一个 batch，不一定是一张图片。

3. epoch = 整个训练集被遍历一次

   假如训练集1009张图片，batch_size=100，在这一个epoch中，前向传播、反向传播、参数更新都是 10次
...
剩下的懒得看了直接看：https://github.com/AccumulateMore/CV/blob/main/118_%E7%BD%91%E7%BB%9C%E6%A8%A1%E5%9E%8B%E4%BF%9D%E5%AD%98%E4%B8%8E%E8%AF%BB%E5%8F%96.ipynb

**网络模型保存（方式1）模型结构+模型参数**
